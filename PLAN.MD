...
- Multi-user: Basic auth so each user has their own isolated todo universe, each user has its profile and settings page
- User can see previous buckets and statistics in a Retrospective page
...
- TypeScript
- Feature based folder structure (server code co-located into the feature folders)
...
## [DONE] Phase 2: Client-Side State Management
...
## [DONE] Phase 3: Database Schema & Drizzle Setup
...
- schema.ts with:
  - users table
  - buckets table (id, type, period, status, userId)
  - todos table (id, title, bucketId, userId, completed, createdAt)
...
## [DONE] Phase 4: Server Actions & Data Fetching
...
## [DONE] Phase 5: Authentication with BetterAuth
...
- BetterAuth handles cookie management and session validation
