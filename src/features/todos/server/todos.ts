...
import { TodoDbInsert } from '@/features/shared/db/types'
...
export const createTodo = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    return await db.insert(todos).values(newTodo) // TODO check if I need the returning value since I'm then doing the mutation on the client
  })
...
export const deleteTodo = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    return await db.delete(todos).where(eq(todos.id, data.id))
  })
