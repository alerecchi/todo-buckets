...
export function BucketColumn({ bucket, onAddTodo, onRemoveTodo, onToggleTodo }: BucketProps) {
  const [isDialogOpen, setDialogOpen] = useState(false)

  const { data: todoList = [] } = useSuspenseQuery(getTodosQueryOptions(bucket.id))

  return (
    <div className='flex min-h-[100px] flex-col gap-4 rounded-lg border-2 border-dashed border-border bg-muted/20 p-2'>
      <div className='flex items-center justify-between'>
        ...
        <AddTodoDialog isOpen={isDialogOpen} setOpen={setDialogOpen} onAddTodo={onAddTodo} />
      </div>
